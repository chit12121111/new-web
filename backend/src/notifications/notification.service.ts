...
export class NotificationService {
  ...
  constructor() {
    ...
    if (emailHost && emailUser && emailPass) {
      ...
      this.logger.warn('📧 Email notifications disabled (missing EMAIL_* env vars)');
    }

    this.discordWebhookUrl = process.env.DISCORD_WEBHOOK_URL;
    if (this.discordWebhookUrl) {
      this.logger.log('🔔 Discord notifications enabled');
    } else {
      this.logger.warn('🔔 Discord notifications disabled (missing DISCORD_WEBHOOK_URL)');
    }
  }
  ...
  private async sendEmailNotification(payload: ImageNotificationPayload) {
    ...
    try {
      await this.emailTransporter.sendMail({
        from: this.emailFrom,
        to: this.emailTo,
        subject: `🖼️ Image generated by ${payload.userEmail}`,
        html: `
  ...
  private async sendDiscordNotification(payload: ImageNotificationPayload) {
    ...
    try {
      await axios.post(this.discordWebhookUrl, {
        content: `🖼️ **${payload.userEmail}** generated an image`,
        embeds: [
          {
